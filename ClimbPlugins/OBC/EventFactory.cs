            ...
            if (MyModules.ContainsKey(moduleId))
            {
                ...
                if (evt != null)
                {
                    ...
                    foreach (var p in evt.Pars)
                    {
                        ...
                        {
                            ulong value = BitConverter.ToUInt64(rxData.AsSpan(dataIdx, 8));
                            dataIdx += 8;
                            retVal += p.Short + ":" + value.ToString() + " ";
                        }
                        else if (p.Type == "uint32")
                        {
                            uint value = BitConverter.ToUInt32(rxData.AsSpan(dataIdx, 4));
                            dataIdx += 4;
                            retVal += (string.IsNullOrWhiteSpace(p.Short) ? "" : p.Short + ":") + value.ToString(p.Format) + " ";
                        }
                        else if (p.Type == "uint16")
                        {
                            ushort value = BitConverter.ToUInt16(rxData.AsSpan(dataIdx, 2));
                            dataIdx += 2;
                            retVal += p.Short + ":" + value.ToString() + " ";
                        }
